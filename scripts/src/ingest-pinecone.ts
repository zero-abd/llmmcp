 ...
 */
if (process.env.LLMMCP_API_URL && process.env.LLMMCP_API_KEY) {
  console.log("\nTriggering Worker cache refresh...");
  await triggerRefresh(process.env.LLMMCP_API_URL, process.env.LLMMCP_API_KEY);
} else {
  console.log("\nSkipping Worker refresh (LLMMCP_API_URL or LLMMCP_API_KEY not set)");
}
